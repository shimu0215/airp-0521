...
from loss_function import energy_force_npa_mse_Loss
...
# checkpoint = torch.load('saved_model/PaiNN-0818.pth')
...
loss_npa_energy_force = energy_force_npa_mse_Loss()

writer = SummaryWriter(log_dir=f"log/0819_mse_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
...
def train(data):
    ...
    #     species=data.species.to(device).to(torch.long)[node_mask.view(-1)].clone()
    # )
    pred = model(data)
    ...
    with torch.no_grad():
        ...
        if best_val_loss >= val_loss:
            ...
            torch.save({'model_state_dict': model.state_dict(),
                        'optimizer_state_dict': optimizer.state_dict(),}, path + 'PaiNN-0819_mse.pth')
        else:
            patience -= 1
            decay_patience -= 1
